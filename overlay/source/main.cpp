...
class WANToggleListItem : public tsl::elm::ToggleListItem {
public:
  WANToggleListItem() : ToggleListItem("WAN", false) {
    u32 enabled;
    Result rc;

    rc = ldnMitmGetWANEnabled(&g_ldnConfig, &enabled);
    if (R_FAILED(rc)) {
      g_state = State::Error;
    }
    ...
    this->setStateChangedListener([](bool enabled) {
      Result rc = ldnMitmSetWANEnabled(&g_ldnConfig, enabled);
      if (R_FAILED(rc)) {
        g_state = State::Error;
      }
    });
...
public:
    ...
    virtual tsl::elm::Element* createUI() override {
        ...
        if (g_state == State::Error) {
            ...
            list->addItem(new WANToggleListItem());
